...
#define ERROR_GRID_SIZE 1
#define ERROR_NUMTASKS	2
#define INV_ARG_SIDE	3
#define INV_ARG_STEPS	4
...
void error_handler(int error_code){
	...
	else if (error_code == ERROR_GRID_SIZE){
		fprintf(stderr, "The grid _size must be divisible with the number of the processes \n");
		MPI_Finalize();
		exit(1);
	}
	else if (error_code == ERROR_NUMTASKS){
		fprintf(stderr, "The number of the processes must be square number \n");
		MPI_Finalize();
		exit(1);
	}
	else if (error_code == INV_ARG_SIDE){
		fprintf(stderr, "The square grid's side size must be a positive integer number\n");
		MPI_Finalize();
		exit(1);
	}
	else if (error_code == INV_ARG_STEPS){
...
void checks(){
	...
	if (info.grid_size%numtasks != 0)
		error_handler(ERROR_GRID_SIZE);

	if (!is_square_number(numtasks))
		error_handler(ERROR_NUMTASKS);
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	/*  The necessary checks of the arguments.
	*/
	if (info.side_size <= 0)
		error_handler(INV_ARG_SIDE);

	if (steps <= 0)
		error_handler(INV_ARG_STEPS);
	...
	if (taskid == MASTER){
		...
		/* Print the initial grid if you like..*/
	...
	*/
	...
	if (taskid == MASTER) {
		...
		/* Print the final grid if you like..*/
